...
No build step required -- the app deploys directly from source code.
...
- Upload source code directly to workspace (no build step needed)
...
├── pyproject.toml              # Python project config (for local dev with apx)
├── src/
│   ├── app.yml                 # App runtime config (uvicorn entrypoint + env vars)
...
│   │   │   ├── core/           # App framework (static files, headers, config)
│   │   │   └── routes/         # API routes (catalog, profiling, metadata, apply, genie)
│   │   ├── __dist__/           # Pre-built React frontend (served by FastAPI)
│   │   └── ui/                 # React frontend source (TypeScript + Vite)
│   └── notebooks/
...
The app deploys directly from `src/` -- no intermediate build step. `databricks bundle deploy` uploads the source to the workspace and `apps deploy` starts the app from there.
